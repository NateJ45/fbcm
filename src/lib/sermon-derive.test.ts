...
test('readingOf finds the scripture reference, in the forms the previews use', () => {
  assert.equal(
    readingOf(
      '...kingdom of heaven. The one who can accept this should accept it." - From Matthew 19:1-14 (NIV)',
    ),
    'Matthew 19:1-14',
  );
  assert.equal(
    readingOf('"So Peter was kept in prison..." -excerpt from Acts 12:1-19 Ever tried'),
    'Acts 12:1-19',
  );
  assert.equal(
    readingOf('This week we read 1 Thessalonians 5:12-28 together'),
...
test('readingOf never guesses', () => {
  assert.equal(
    readingOf('The book of Ecclesiastes is perhaps the only true book of philosophy'),
    '',
  );
...
test('seriesOf reads the series name out of the opening sentence', () => {
  assert.equal(
    seriesOf(
      'This is a sermon preview for the second week of our Kingdom Family Values series. Visit',
    ),
    'Kingdom Family Values',
  );
  assert.equal(
    seriesOf('the first of four sermons in FBCM’s "Multiplied" sermon series.'),
    'Multiplied',
  );
