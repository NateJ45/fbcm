...
const norm = (u) => {
  try {
    const p = new URL(u);
    let s = p.pathname.replace(/\/+$/, '').toLowerCase();
    return (s === '' ? '/' : s);
  } catch { return null; }
};
...
const cdxPage = async (host, resumeKey, limit) => {
  const url = `http://web.archive.org/cdx/search/cdx?url=${host}*&output=json`
  ...
  for (let page = 0; page < 60; page++) {
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
      if (rows[i].length === 0 || (rows[i].length === 1 && !rows[i][0])) {
        const tail = rows.slice(i + 1).flat().filter(Boolean);
...
const hist = new Map();
const byYear = {};
let totalCaptures = 0, first = null, last = null;
...
for (const [original, ts, status, mime] of raw) {
  ...
  if (!key) continue;
  const e = hist.get(key) ?? { url: key, firstSeen: ts, lastSeen: ts, captureCount: 0, sample: original };
  e.captureCount++;
  if (ts < e.firstSeen) e.firstSeen = ts;
  if (ts > e.lastSeen) { e.lastSeen = ts; e.sample = original; }
  hist.set(key, e);
}

// --- current ----------------------------------------------------------------
const currentRaw = (await Promise.all([
  'pages-sitemap.xml',
...
const gone = [...hist.values()]
  ...
  .map((e) => ({ ...e, firstSeen: iso(e.firstSeen), lastSeen: iso(e.lastSeen),
...
await writeFile(path.join(OUT, 'wayback-inventory.json'), JSON.stringify({
  domain: 'fbcmuncie.org',
  cdxQueriedAt: new Date().toISOString(),
  totalCaptures, firstCapture: first && iso(first), lastCapture: last && iso(last),
  capturesByYear: byYear,
  counts: { historicalUrls: hist.size, currentUrls: current.size, gone: gone.length, new: newUrls.length },
  gone, newUrls,
  historicalUrls: [...hist.values()].map((e) => ({ ...e, firstSeen: iso(e.firstSeen), lastSeen: iso(e.lastSeen) })),
  currentUrls: [...current].sort(),
}, null, 2) + '\n', 'utf8');

console.log('html 200 captures:', totalCaptures, '| first:', first && iso(first), '| last:', last && iso(last));
console.log('by year:', JSON.stringify(byYear));
console.log('historical html urls:', hist.size, '| current:', current.size, '| GONE:', gone.length, '| new:', newUrls.length);
console.log('\n--- gone, most-captured first (top 40) ---');
for (const g of gone.slice(0, 40)) console.log(String(g.captureCount).padStart(4), g.lastSeen, g.url);
