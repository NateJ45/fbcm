...
console.log(
  `manifest: ${manifest.files.length} files, ${(manifest.totals.bytes / 1e9).toFixed(2)} GB`,
);
...
for (const entry of manifest.files) {
  ...
  if (s.size !== entry.bytes) {
    wrongSize.push(`${entry.path} (${s.size} vs ${entry.bytes})`);
    continue;
  }
  if (!quick && (await sha256(full)) !== entry.sha256) {
