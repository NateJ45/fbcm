...
- Robustness: the global and in-reference "Create new" menus no longer offer a singleton (a button link could create a second Home page) or a blank Saved section; "Location details" (businessInfo) left the desk, since opening it created a document with the starter's "Your City, XX", which Sunday times prints after the address; every filtered desk list carries an `apiVersion`; the embed section's "not both" check read the page instead of the section, so it never fired; a button set to "A page on this site" with no page picked, and a phone button with no number, are now errors.
- Dead controls hidden, not removed (rule 1): a page's "Show in the top menu" and "Where in the top menu" (nothing reads them; the menu is Site settings), the Blog page's photo, accent and whole closing band, Privacy's photo and accent, the 404 photo, a ministry's summary, a post's related posts and cover caption, and the newsletter signup (it needs a developer, and the site draws no form of its own). The Blog page's floating-button description was wrong: blank shows "Got a question?", it cannot hide the button.
- The Help panel "Your church at a glance" read the empty legacy social fields; it now shows the service time, the address and the real social accounts.
- Guides: every backticked name matches the Studio, "band" became "section", and a new guide, "What the home page fills in by itself", says where Last Sunday, The Visitor, What's On, the church app and the dated Sunday line come from. `src/lib/studio-guide-names.test.ts` is the gate: a guide that names a box, a desk item or a button the Studio does not show fails the unit tests.

_2026-09-25 — People in arches: Wedding's coordinator, the Staff hero, Contact's co-pastors (`worktree-agent-a432365ea207c703b`)._

**Four fixes Nathan asked for, three of them general.** (1) A portrait beside a few words was hung tall and far from them, the heading alone on the row above: `besideForm()` in `src/lib/photo-shape.ts` makes such a band **compact** (at most 70 words, no small headings: the heading moves into the text column, the words centre on the portrait, the arch is capped at 22rem, words 1/7 and portrait 8/13), which recomposes Wedding's coordinator and History's Saunders band alike; `photoSubject()` now reads a role ("wedding coordinator") and "portrait" as people, so Ella Mae Lemen's portrait is arched instead of standing in the tower lancet. (2) /staff drops "A Note From Our Pastors" (`scripts/pages/staff.mjs`; the letter stays on /who-we-are). (3) The Staff hero's square headshots in 100/260 lights lost hair and forehead to the point: `archPlacement()` in `src/lib/hero-frames.ts` gives a close people portrait in a lancet that draws it at least twice the arch's width, with its hotspot above the springing, headroom so the hotspot lands on the face line; ArchFrame also maps the hotspot through a crop now. (4) Contact's pastor band shows both co-pastors: `contact.mjs` sets Jonathan's staff portrait as the band's `detail`, and two people portraits draw as a **pair** of equal lancets (the w6 row split). No schema field; the `detail` field's Studio description says so. staff.mjs and contact.mjs wait on a deploy and `--apply`.
...
- **Home's What's On band** (`src/components/home/WhatsOnBand.astro`, `nextEvents()`): the next three dated events after The Visitor in the insert slot, Last Sunday's split, each row a small date tile, the title linking to its row on /events, and the day, time and room; its ground is the opposite of the band above by The Visitor's rule, decided in `index.astro`. No band when nothing dated is ahead. `last-sunday.spec.ts` and `visitor.spec.ts` step over it.
...
- **"Let us know you’re coming"** replaces "Fill in a visitor card" on the Visit hero's gold button and the closing gold band. Both still point at `{connect}` (Site settings > Church systems > connection card) and open in a new tab. The promise ("If you let us know you’re coming, a greeter will look out for you.") is new copy on the approval note, with the question the church has to answer before it goes live: who reads the card, and do they tell the greeters?
- **Good to know**, a Text block under the hero, answers four of the church's own FAQ questions in the open as Ledger columns: what to wear, "I’m not sure what I believe", communion, a child's Bible. The full FAQ stays. "Will I be singled out?" and "Do I have to give?" are not answered anywhere in the church's content, so they are questions in PENDING, not sentences on the page.
- **Your children**, a Text block straight after the morning path (`#children`), carries the safety facts that were only in the Ministries FAQ: /children's own lede (background checks, pick-up only with a matching security tag), then Check-in, Who cares for them, Pick-up, and Ages and rooms. Every sentence is read off `faq-entries.json` and the captures and throws if it moves. The first render mixed columns over and under the Ledger's 40-word "big" line and left a short last line as a stray foot; each column is now one paragraph of 40 words or fewer.
- **No past events on Home's Church Blog rows** (`src/lib/past-events.ts`, rule 15: derived from the post, no field). An FBCM Events post drops from Home once the latest date it writes down has passed (a year-less date resolved against the publication day; else a year in the title; else 180 days). Tested against the church's real 13 event posts: on 25 September 2026 it keeps the Messiah Sing-In (11 December 2026) and hides the other 12, including "Lent & Easter 2026" (ends 18 April, the Easter open house) and Blue Christmas (9 December 2025). The journal projection carries the body as plain text for it. /blog stays the complete register. Home now shows the Messiah Sing-In, the September 2025 job listing and the January 2026 Ecclesiastes preview.

Gates: check 0 errors (the known `scaffold.mjs` lint warning), format clean, 1222 unit tests (94 files; `past-events` new), 52 script tests, `audit:studio` clean, build with the rule 20 grep empty. Parity against main's baselines was 167/170: Home (two rows replaced), `/styleguide` (the Blue Christmas row gone from its fixed band) and `/styleguide/visit` (a stale baseline from the old Visit fixture, replaced by the new composition); no inline sheet changed size (Home 166,936 B both sides). Recaptured, rebuilt, 170/170. Playwright 485 passed, 2 skipped, with the new `tests/visit.spec.ts` (Good to know and Your children visible with nothing to expand, the button's href, target and rel, axe, no overflow at 320, and the styleguide band never showing the past event). The dry run and a keyed field diff of the live document are in PENDING.

_2026-09-25 — This Sunday's preacher, and what the service is like (`feat/preacher-and-feel`)._

**The home hero names who is preaching.** A fourth fact after Sundays, Where and Online: "Preaching / Rev. Jonathan Balmer". Nothing is typed (rule 15). `src/lib/preacher.ts` picks it in the dated line's order: the author of the sermon preview for the coming Sunday (the post page already prints the author as "Preaching"; "FBC Muncie", the church's own account on 11 previews, names nobody), else the "Preaching:" line of the broadcast YouTube has scheduled for that Sunday, else nothing, and the row is the old three. The parser accepts "Preacher:", "Preaching:" and "Speaker:" at a line or a `|`/`•`/`·` segment, keeps titles as written, and names nobody for "TBA", a link, a sentence or a bare title; the Last Sunday band reads its preacher line through the same parser. The fact is not on the dated line, whose sermon half is held to one 320 px line. Four facts are a 2 x 2 grid below 1280 px and one row above it. Measured: the stacked row put the name below a 320 x 640 first screen, and at 1440 the flex row left it alone on a second line. Three facts keep the old class string.

**Stale-safe, with or without a script.** The fact is server-rendered `hidden` with its Sunday in `data-sunday-fact`; BaseLayout's upgrade script shows it only while `sundayFactKept` holds (the Sunday the line names is its Sunday) and is called from inside the fact, before first paint. A page that outlives its Sunday, and a visitor without JavaScript, never see a name that cannot be dated. `live-sunday-inline.test.ts` runs the inline copy against the module over three weeks of hours.

**What the service is like, in the church's words, waiting on the church.** One schema field, `heroFact.note` (optional, 48 characters), drawn under the value in the photo hero. `home.mjs` proposes "Intergenerational, casual dress welcome" for the Sundays fact: the Worship goal's "the full, intergenerational body of Christ" and the FAQ's "Casual dress is welcome.", both guarded to throw if they move. "About an hour" was left out: `serviceLength` is the schema default, never the church's word, and the replays run 10:40 to about noon. "Hymns" was left out too: only our own Visit lede says it. The dry run changes exactly one live field (`facts[fact-1].note`). The line is on the approval note, and the apply waits on the deploy (rule 1) and the church.

Gates: check 0 errors (the known `scaffold.mjs` lint warning), format clean, 1216 unit tests (94 files, 18 new), 52 script tests, build with the rule 20 grep empty. Parity: 1/169 against main's baselines. Every page carries the inline script's new lines. Home and the three existing `/styleguide/this-sunday` states carry the hidden preacher fact. Visit carries live content another branch applied at 12:07Z. After the grid, every page carries the stylesheet's +326 B (129,713 B to 130,039 B). Recaptured, rebuilt, 172/172 PASS at 130,039 B on both sides. Playwright 473 passed, 2 skipped (12 new).
