...
- `church/TimelineContents.astro` -- **the contents bar** (2026-09-25, `feat/history-contents`; today /history only). A timeline whose rows link to at least three bands on the same page (History's "Read this era" links) is a table of contents, and SectionRenderer draws a sticky indigo bar with a gold rule just before it. Everything is derived in `src/lib/timeline-contents.ts` (unit-tested, stega-safe, no schema field): an entry per row whose body links to a band that exists below the timeline (the Today row links to /staff, so it is not one), its year the marker's first four-digit year, the first link the timeline itself by its heading, and the span from the timeline to the last entry's band plus any headless bands after it (prose tails, pull quotes). Wide (>= 1024px): the heading in Sofia capitals, one Castoro Titling year per era, the era in view gold with a gold rule and `aria-current="location"`, its title in italics at the right; each link's accessible name is the whole marker and title ("1859 to 1862: Founding"). Narrow: the era in view and a "Jump to" `<details>` (no script needed to open it) whose list drops over the page in 48px rows; a jump, Escape or a tap outside closes it. It is `position: sticky` in `#main`, at the top of the window while the header is away and under it (`--header-h`, sliding with it) while the header is pinned; one IntersectionObserver on a one-pixel line under the landing point marks the part in view, and past the span the bar steps aside. Every anchor under `#main` gets scroll-margin for the header offset plus the bar; because the header leaves on a scroll down, a click on a same-page link going DOWN sets `--hc-land` to the bar plus the header's air for that jump (capture phase, before the browser's own jump), so the heading lands under the bar instead of under a header-sized gap. Its CSS is `is:inline` inside the bar so it ships only on the page that draws one (a bundled style rode in the shared SectionRenderer chunk on every page). `tests/history-contents.spec.ts`; `tests/anchors.spec.ts` measures below the bar when it is stuck. Why not a side rail: the bands are full-bleed grounds with photographs bled to the edge and a 1400px content column, so up to 1544px there is no gutter for one.
...
- `sections/ImageText.astro` -- people photos in arches: a row in a door (a wide door for landscape), a portrait in a lancet (the tower `Lancet.astro` stays for a portrait of a place); buildings stay rectangles; plate, legend and ground untouched. The optional `detail` photo sits in a small lancet over the main photo's corner (a portrait carrying one takes the door so the arches differ). A body whose every h3/h4 names a room in brackets draws as a room board (`roomBoard()`). h2 on `H2_DISPLAY`, button the gold plate. `photo-shape.ts` now never makes a people photo a ground, so the ground budget counts places only. Since 2026-09-25 a beside band also has a **composition** (`besideForm()` in `photo-shape.ts`, unit-tested): **compact** (a portrait beside at most 70 words with no small headings: the heading moves into the text column, the words centre on the portrait, the arch is capped at 22rem, words 1/7 and portrait 8/13 whatever the shape; Wedding's coordinator) and **pair** (the photo and its `detail` both portraits of people: two equal lancets in the w6 row split, fig 7/13 and words 1/6; Contact's co-pastors), else standard. `photoSubject()` reads a role (coordinator, director, secretary, ...) and "portrait" as people, so a staff portrait whose alt is only a name and a role is arched.
- `church/ArchFrame.astro` -- since 2026-09-25 places its photo through `archPlacement()` (`src/lib/hero-frames.ts`, unit-tested): the hotspot mapped through any crop (else the top), and HEADROOM for a close portrait of people (aspect at most 1.1 after the crop) in a lancet tall enough to draw it at least twice the arch's width (height/width x aspect >= 2; the ratio comes from the `style` prop's `--arch-ratio`, or a `frameRatio` prop, which the window hero passes as 2.6 and 1.4 for its lights) whose hotspot sits above `LANCET_FACE_LINE` (0.5): the photo is drawn in the arch's lower part so the hotspot lands on the line, fading in over its top 6%, capped at 30%. A portrait fills the arch's height, so no object-position could move a head clear of the point. No hotspot, a door, a place, a landscape or a 100/150 lancet: unchanged (History's opener portrait must not move). The Staff hero's two co-pastor headshots are the case it was written for.
