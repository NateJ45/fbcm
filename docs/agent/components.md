...
- `church/ArchFrame.astro` -- since 2026-09-25 places its photo through `archPlacement()` (`src/lib/hero-frames.ts`, unit-tested): the hotspot mapped through any crop (else the top), and HEADROOM for a close portrait of people (aspect at most 1.1 after the crop) in a lancet whose hotspot sits above `LANCET_FACE_LINE` (0.5): the photo is drawn in the arch's lower part so the hotspot lands on the line, fading in over its top 6%, capped at 30%. A portrait fills the arch's height, so no object-position could move a head clear of the point. No hotspot, a door, a place or a landscape: unchanged. The Staff hero's two co-pastor headshots are the case it was written for.
