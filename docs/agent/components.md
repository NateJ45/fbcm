...
- `CtaLink.astro` `variant="rule"` -- the church identity button: square, solid fill, a 3px gold rule 3px beneath, three small rays that spread on hover and focus. Brand indigo with a white label on paper (hover indigo-dark); with `onDark`, brand gold with indigo-dark ink and a paper rule (hover `--color-gold-hover`). Its geometry (`.btn-rule`) is unlayered CSS, so utility classes passed in `class` cannot resize it.
