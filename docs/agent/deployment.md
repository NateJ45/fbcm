...
### IndexNow (2026-09-24, the local search pass)

IndexNow is how a site tells Bing (and Yandex, Seznam, Naver and the other engines that share submissions) that its pages exist or changed, instead of waiting to be crawled. Bing's index also feeds Copilot and ChatGPT search, so it matters for the AI answers as much as for Bing itself.

- **The key** is `site.indexNowKey` in `src/data/site.ts` (32 hex characters, not a secret). `src/pages/[indexNowKey].txt.ts` publishes it at `<site.url>/<key>.txt`, at the root, because a key file only vouches for URLs at or below its own directory.
- **The script** is `scripts/indexnow.mjs` (`npm run indexnow`; `-- --dry-run` checks the gate and prints the plan without POSTing). Its pure half, `scripts/lib/indexnow.mjs`, is tested by `npm run test:scripts`. It reads the sitemap this build produced (`dist/client/sitemap-index.xml` and the sitemaps it names; the live sitemap when there is no build), keeps the URLs on `site.url`'s host, and POSTs them to `https://api.indexnow.org/indexnow` with the host, the key and the key's location, 10,000 URLs a request at most. 200 and 202 are success.
- **The gate.** It submits nothing, and exits 0 saying why, unless `<site.url>/<key>.txt` answers 200 with exactly the key. Before the cutover `www.fbcmuncie.org` is still Wix, which answers 400 for that path, so today every run is a logged no-op. The first deploy after the domain moves opens the gate on its own; the cutover plan's "Search and AI visibility at cutover" list says how to confirm that first run.
- **Where it runs.** `.github/workflows/deploy.yml`, the last step, after the smoke (a deploy that failed its smoke is never advertised), with `continue-on-error: true`: a search engine refusing a submission must never fail a deploy. Exit 1 means the gate was open and a POST failed; the step turns red without failing the run, and its log carries the engine's answer.
- **What it submits: every sitemap URL, every deploy.** The sitemap has no dates to tell a changed page from an unchanged one, and a few hundred URLs is far inside the protocol's limits; the engines ignore URLs that have not changed. A deploy fires on every Studio publish, so this is a few submissions a week. If that ever needs narrowing, the thing to build is a changed-pages list (hash each built page, keep the previous deploy's hashes as a published file, submit the difference).
