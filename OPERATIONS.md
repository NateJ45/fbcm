...
- [ ] `npm run parity:compare` is green against the committed baselines in `scripts/.parity/` (162/162 on FBCM as of 2026-09-20). Build first; the harness never builds. Run it on any change meant to be render-neutral, and recapture only deliberately, once, when the pages have stopped moving. **Read `docs/PENDING.md` known gap 2 before you trust a green compare here:** the baselines are committed `.html`, so Tailwind scans them, so a capture feeds the next build's stylesheet, and cutting that loop has to come before the next recapture.
- [ ] `npx playwright test tests/anchors.spec.ts` passes on both projects (11 passed, 1 skipped by design on webkit). It asserts real geometry: that a fragment landing sits below the sticky header, on desktop and on the iPhone profile, including under reduced motion. A failure here is a real finding, never a tolerance to loosen.
- [ ] The domain move itself follows `docs/superpowers/notes/2026-09-20-cutover-plan.md`: the dry output of `node scripts/cutover.mjs`, the human prerequisites with an owner on each line, and the after-the-move verification. Read the dry plan before passing `--write`; without `--zone-file` the script adds no DNS records at all, which takes the client's mail with it.
...
| 4     | Events calendar (`{calendar}`)             | `https://www.churchtrac.com/public_calendar?ui=0C7B1090` (or `https://fbcmuncie.churchtrac.com/upcoming_events`)           | /events ("What's On") reads its calendar code from this box, and its "Open the calendar" door; add `{calendar}` to a link where wanted                                  |
...
| Inline scripts blocked, theme/layout/polish all break   | Someone enabled `security.csp` in `astro.config.mjs`                                         | Remove the config block. See CLAUDE.md → Stack → Astro config don'ts.                                                              |
